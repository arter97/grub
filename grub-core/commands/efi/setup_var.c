...
static grub_err_t grub_cmd_setup_var(grub_command_t cmd, int argc, char *argv[])
{
	...
	if (cmd->name[CMDCHECK_SETUP_VAR2] != 0)
		isMode2 = 1;
	...
	if (argc == 0) {
		...
		grub_printf("RESPONSIBILITY FOR  Y O U R  ACTIONS.\n");
		grub_printf
	...
	do {
		...
		status =
		    efi_call_3(grub_efi_system_table->runtime_services->
			       get_next_variable_name, &name_size, name, &guid);

		if (status == GRUB_EFI_NOT_FOUND) {	/* finished traversing VSS */
			break;
		}
		...
		if (!status
		    &&
		    ((name_size == INSYDE_SETUP_VAR_NSIZE
		      && 0 == grub_memcmp(name, INSYDE_SETUP_VAR, name_size))
		     || (isMode2 && name_size == INSYDE_CUSTOM_VAR_NSIZE
			 && 0 == grub_memcmp(name, INSYDE_CUSTOM_VAR,
					     name_size)))) {
			grub_printf("var name: ");
			print_varname(name);
			grub_printf
			    (", var size: %u, var guid: %08x-%04x-%04x - %02x-%02x-%02x-%02x-%02x-%02x-%02x-%02x\n\n",
			     (grub_uint32_t) name_size, guid.data1, guid.data2,
			...
			if (grub_memcmp
				...
				grub_memcpy(&setup_var_guid, &guid,
...
/* obtain current contents of Setup variable */
			if (argc >= 1 && argc < 3) {
				...
				if (endptr == argv[0] || grub_errno != 0) {
					return grub_error(GRUB_ERR_BAD_ARGUMENT,
							  "can't decode your first argument. Please provide a hex value (e.g. 0x1af).");
				}
				status =
				    efi_call_5(grub_efi_system_table->
					       runtime_services->get_variable,
					       name, &setup_var_guid,
					       &setup_var_attr, &setup_var_size,
					       tmp_data);
				if (status == GRUB_EFI_BUFFER_TOO_SMALL) {
					grub_printf
					    ("expected a different size of the Setup variable (got %d (0x%x) bytes). Continue with care...\n",
					     (int)setup_var_size,
					     (int)setup_var_size);
					status =
					    efi_call_5(grub_efi_system_table->
				...
				if (status) {
					return
					    grub_error(GRUB_ERR_INVALID_COMMAND,
						       "can't get variable using efi (error: 0x%016x)",
						       status);
				}
				grub_printf
				    ("successfully obtained \"Setup\" variable from VSS (got %d (0x%x) bytes).\n",
			...
			if (argc == 2) {
				...
				if (status) {
					return
					    grub_error(GRUB_ERR_INVALID_COMMAND,
						       "can't set variable using efi (error: 0x%016x)",
						       status);
				}
			}
	...
	if (argc == 0 || argc > 2) {
		return grub_error(GRUB_ERR_BAD_ARGUMENT,
				  "Usage: %s offset [setval]", cmd->name);
	}
	return grub_errno;
}
		  ...
		  __attribute__ ((unused)))
{
	...
	do {
		...
		if (!status) {
			setup_var_size = 1;
			status =
			    efi_call_5(grub_efi_system_table->runtime_services->
				       get_variable, name, &guid,
				       &setup_var_attr, &setup_var_size,
				       tmp_data);
			if (status && status != GRUB_EFI_BUFFER_TOO_SMALL) {
				grub_printf
				    ("error (0x%x) getting var size:\n  ",
				     (grub_uint32_t) status);
				setup_var_size = 0;
			}
			status = 0;
			...
			grub_printf
			    ("name size: %02u, var size: %06u (0x%06x), var guid: %08x-%04x-%04x - %02x-%02x-%02x-%02x-%02x-%02x-%02x-%02x, name: ",
			     ...
			     (grub_uint32_t) setup_var_size, guid.data1,
			     guid.data2, guid.data3, guid.data4[0],
...
GRUB_MOD_INIT(setup_var)
{
	...
	cmd_setup_var2 =
	    grub_register_command(CMDNAME_SETUP_VAR2, grub_cmd_setup_var,
				  "setup_var2 offset [setval]",
				  "Read/Write specific (byte) offset of setup and custom variables.");
	cmd_setup_lsvar =
	    grub_register_command("lsefivar", grub_cmd_lsefivar, "lsefivar",
